function J=evaluator1(ind,parameters,i,fig)
...
try
    ...
    f=@(T,Y) Dynamic_model_not_lin(T,Y,b(Y(1:2)),Q,R,100);
    opts = odeset('RelTol',1e-11,'AbsTol',1e-14);
    [T,Y]=ode45(f,[0:dt:Tf],[x0 ;0]',opts);
    L=length(T);

 for k =3:L
    filtVel(k)=Dnum*Y((k-[0:2]),1)-Dden(2:3)*filtVel(k-[1:2])';
 end
    if T(end)==Tf
        J=Y(end,3);
    end
...
%     m1='-4.381112427840022e+03*S0 -24.633634789067290*S1';
 ...
 m1='-247.7*S0 -5.418*S1';
    ...
    f=@(T1,Y1) Dynamic_model_not_lin(Y1,b1(Y1(1:2)),Q,R,1000);
    opts = odeset('RelTol',1e-11,'AbsTol',1e-14);
