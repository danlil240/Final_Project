function J=evaluator2(ind,parameters,i,fig)
YY = 10
...
try
    ...
    f=@(T,Y) Dynamic_model_lin_mod2(T,Y,min(max(b(Y(1:2)),-2),2),Q,R,Tevmax);
    opts = odeset('RelTol',1e-5,'AbsTol',1e-8);
    [T,Y]=ode45(f,[0:dt:Tf],[x0]',opts);

    %  [T,Y]=ode45(@(T,Y) Dynamic_model_lin_mod2(T,Y,min(max(b(Y(1:2)),-2),2),eye(2),1,0),0:0.001:0.4,[1e-5,0,0,0,0]');
    if T(end)==Tf
        J=Y(end,3);
    end
catch
...
if nargin>3
    ...
    f=@(T1,Y1) Dynamic_model_lin_mod2(T1,Y1,min(max(b1(Y1(1:2)),-2),2),Q,R,Tevmax);
