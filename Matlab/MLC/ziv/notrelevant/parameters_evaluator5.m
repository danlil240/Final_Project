...
%Type + to create corresponding MLC object
%  mlc=MLC('parameters_evaluator5')
% mlc.go(n)   mlc.go(50,1)
% mlc.show_best
% mlc.show_ind(n2);  man(n2) = mlc.table.number;
% readmylisp_to_formal_MLC('(+ (* S1 -1.568) (* (+ S1 S0) (* S1 -25.72)))')
% syms S0 S1
% expand    simplify
...
parameters.range = 200;%10000;
parameters.precision = 4;
parameters.opsetrange = [1 3];%[1 2 3];
parameters.formal = 0;
parameters.end_character = '';
parameters.maxdepth = 2;%15;
parameters.maxdepthfirst = 2;%8;
...
parameters.evaluation_function='evaluator5';
...
parameters.verbose = 5;%3;
parameters.fgen = 250;
parameters.show_best = 1;
parameters.problem_variables.Tf=0.4;
parameters.problem_variables.Tevmax=15;
parameters.problem_variables.R=0.1;%0.001;
parameters.problem_variables.Q=diag([22000 0.1 0 0]);%Q=diag([1000 0.5]);%
parameters.problem_variables.dt=0.001;%0.001;
parameters.problem_variables.Tevmax=10;
parameters.problem_variables.x0=[5e-5;0;0;0];
