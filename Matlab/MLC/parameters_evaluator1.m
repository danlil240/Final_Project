...
%Type mlc=MLC('parameters_evaluator1') to create corresponding MLC object
% mlc.go(n)   mlc.go(50,1)
% mlc.show_best
% mlc.show_ind(n2);  man(n2) = mlc.table.number;
% readmylisp_to_formal_MLC('(+ (+ S0 (+ S0 (* (* -12.14 (+ (* (+ S1 -1.68) -7.986) (+ S0 -5.032))) S0))) (+ (* -5.42 (+ S0 S1)) (* (* -12.14 (+ (* (+ (* -9.213 S0) -1.68) -7.986) (+ S0 (+ (* -9.213 S0) -1.68)))) S0)))')
% syms S0 S1
% expand    simplify
...
parameters.range = 5000;%10000;
parameters.precision = 4;
parameters.opsetrange = [1 3];%[1 2 3];
parameters.formal = 0;
parameters.end_character = '';
parameters.maxdepth = 8;%15;
parameters.maxdepthfirst = 4;%8;
