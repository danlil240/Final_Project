function J=evaluator3(ind,parameters,i,fig)
...
try
    ...
    f=@(T,Y) Dynamic_model_lin2Z(Y,b(Y(1:2)),Q,R,Tevmax);
    opts = odeset('RelTol',1e-11,'AbsTol',1e-14);
    [T,Y]=ode45(f,[0:dt:Tf],[x0 ;0; 0; 0]',opts);
    if T(end)==Tf
        J=Y(end,3);
    end
catch
...
%     m1='-4.381112427840022e+03*S0 -24.633634789067290*S1';
    ...
    f=@(T1,Y1) Dynamic_model_lin2Z(Y1,b1(Y1(1:4)),Q,R,100);
    opts = odeset('RelTol',1e-15,'AbsTol',1e-18);
    [T1,Y1]=ode45(f,[0:dt:Tf],[x0 ;0; 0; 0]',opts);
