...
b1=@(y)(y);
eval(['b1=@(y)(' m1 ');']);
f=@(T1,Y1) Dynamic_model_fun(Y1,b1(Y1(1:2)),0,Q,R,10000);
opts = odeset('RelTol',1e-13,'AbsTol',1e-10);
[T1,Y1]=ode45(f,[0:dt:Tf],x0,opts);
figure('DefaultAxesFontName','David','DefaultAxesFontSize',14, 'DefaultAxesFontWeight', 'bold')

subplot(4,1,1)
plot(T1,Y1(:,1),'linewidth',1.2)
legend({'LQR'})
ylabel('displacment[m]');
title("LQR on Linear model",'FontSize',8 )
